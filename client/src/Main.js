...
function Main() {
  ...
  const handleUpdatePostLikes = (likesObj) => {
    setPosts(prevPosts => prevPosts.map(post => ({
      ...post,
  ...
  const handleUpdateCommentLikes = (likesObj, postId, commentId) => {
    setPosts(prevPosts => prevPosts.map(post => ({
      ...post,
      comments: post.id === postId
      ? post.comments.map(comment => comment.id === commentId 
          ? {...comment, likes: likesObj.likes}
          : comment)
      : post.comments
    })))
  }
    ...
    <div style={{ paddingLeft: '20px', paddingTop: '60px'}}>
      ...
      {posts.map(post => 
        <Post 
          ...
          onUpdatePostLikes={handleUpdatePostLikes}
          onUpdateCommentLikes={handleUpdateCommentLikes}
        />)}
